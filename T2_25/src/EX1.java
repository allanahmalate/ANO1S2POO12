...
public class EX1 
{
    ...
    public static float[][] preencherArray(byte l, byte c) throws IOException
    {
        ...
        for(int i=0; i<array.length; i++)
        {
            ...
            for(byte j=0; j<array[0].length; j++)
                array[i][j] = validarFloat(j);      //ERROR: The method validarFloat(byte) int the type EX1 is not applicable for the argument (int)
        }
        return array;
    }
    ...
    public static float validarFloat(byte i) throws IOException
    {
        ...
        do
        {
            System.out.println("Valor da joalharia da " + (i+1) + "a coluna(>0): ");
